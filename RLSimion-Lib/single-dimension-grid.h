...
class CSingleDimensionStateVariableGrid : public CSingleDimensionGrid
{
	STATE_VARIABLE m_hVariable;
public:
	void initVarRange();
	CSingleDimensionStateVariableGrid(int m_hVar, int numCenters, Distribution distr = Distribution::linear);
	CSingleDimensionStateVariableGrid(CConfigNode* pParameters);
...
class CSingleDimensionActionVariableGrid : public CSingleDimensionGrid
{
	ACTION_VARIABLE m_hVariable;
public:
	void initVarRange();
	CSingleDimensionActionVariableGrid(int m_hVar, int numCenters, Distribution distr = Distribution::linear);
	CSingleDimensionActionVariableGrid(CConfigNode* pParameters);
	double getVarValue(const CState* s, const CAction* a);
	CNamedVarProperties& getVarProperties(const CState* s, const CAction* a);
	void setFeatureStateAction(unsigned int feature, CState* s, CAction* a);
	void setOffset(double offset);
