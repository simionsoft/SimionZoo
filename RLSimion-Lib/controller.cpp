...
CLASS_CONSTRUCTOR(CLQRController) : CParamObject(pParameters)
{
	...
	/*m_outputActionIndex = CApp::get()->World.getActionDescriptor()->getVarIndex(outputAction);*/
...
{
	if (CApp::get()->World.getT()== 0.0)
		m_intError= 0.0;

	double error= s->getValue(m_errorVariableIndex);
	double dError = error*CApp::get()->World.getDT();
	m_intError += error*CApp::get()->World.getDT();
...
void CWindTurbineJonkmanController::selectAction(const CState *s,CAction *a)
{
	...
	else Alpha = exp((CApp::get()->World.getDT())*m_CornerFreq);
	...
	double TrqRate;
	TrqRate = (GenTrq - s->getValue(m_T_g_index)) / CApp::get()->World.getDT(); //Torque rate (unsaturated)
	...
	m_IntSpdErr = m_IntSpdErr + SpdErr*CApp::get()->World.getDT();                           //Current integral of speed error w.r.t. time
	...
	double d_beta = (PitComT - s->getValue(m_beta_index)) / CApp::get()->World.getDT();
