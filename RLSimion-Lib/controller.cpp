...
CLASS_CONSTRUCTOR(CLQRController) : CParamObject(pParameters)
{
	...
	/*m_outputActionIndex = CApp::World.getActionDescriptor()->getVarIndex(outputAction);*/
...
{
	if (CApp::World.getT()== 0.0)
		m_intError= 0.0;

	double error= s->getValue(m_errorVariableIndex);
	double dError = error*CApp::World.getDT();
	m_intError += error*CApp::World.getDT();
...
void CWindTurbineJonkmanController::selectAction(const CState *s,CAction *a)
{
	...
	else Alpha = exp((CApp::World.getDT())*m_CornerFreq);
	...
	double TrqRate;
	TrqRate = (GenTrq - s->getValue(m_T_g_index)) / CApp::World.getDT(); //Torque rate (unsaturated)
	...
	m_IntSpdErr = m_IntSpdErr + SpdErr*CApp::World.getDT();                           //Current integral of speed error w.r.t. time
	...
	double d_beta = (PitComT - s->getValue(m_beta_index)) / CApp::World.getDT();
