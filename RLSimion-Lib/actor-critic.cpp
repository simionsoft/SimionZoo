...
void CActorCritic::update(const CState *s, const CAction *a, const CState *s_p, double r)
{
	//sample importance weighting: rho= pi(a_t|x_t) / mu(a_t|x_t)
	//where mu is the behavior from which the sample is drawn

	double sampleWeight = 1.0;
	double behaviorProb, policyProb;
	if (CSimionApp::get()->pSimGod->useSampleImportanceWeights())
	{
		behaviorProb = m_pActor->getActionProbability(s, a, true);
		policyProb = m_pActor->getActionProbability(s,a,false);
		sampleWeight = std::min(1.0,std::max(MIN_PROBABILITY,policyProb / behaviorProb));
	}
