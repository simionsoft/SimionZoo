...
IF (ALLOCATED(SrcOtherStateData%LineTypeList)) THEN
  ...
  IF (.NOT. ALLOCATED(DstOtherStateData%LineTypeList)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOtherStateData%LineTypeList.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
IF (ALLOCATED(SrcOtherStateData%ConnectList)) THEN
  ...
  IF (.NOT. ALLOCATED(DstOtherStateData%ConnectList)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOtherStateData%ConnectList.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
IF (ALLOCATED(SrcOtherStateData%LineList)) THEN
  ...
  IF (.NOT. ALLOCATED(DstOtherStateData%LineList)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOtherStateData%LineList.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
IF (ALLOCATED(SrcOtherStateData%FairIdList)) THEN
  ...
  IF (.NOT. ALLOCATED(DstOtherStateData%FairIdList)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOtherStateData%FairIdList.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
IF (ALLOCATED(SrcOtherStateData%ConnIdList)) THEN
  ...
  IF (.NOT. ALLOCATED(DstOtherStateData%ConnIdList)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOtherStateData%ConnIdList.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
IF (ALLOCATED(SrcOtherStateData%LineStateIndList)) THEN
  ...
  IF (.NOT. ALLOCATED(DstOtherStateData%LineStateIndList)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOtherStateData%LineStateIndList.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
IF (ALLOCATED(SrcOtherStateData%MDWrOutput)) THEN
  ...
  IF (.NOT. ALLOCATED(DstOtherStateData%MDWrOutput)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstOtherStateData%MDWrOutput.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
IF (ALLOCATED(SrcConstrStateData%dummy)) THEN
  ...
  IF (.NOT. ALLOCATED(DstConstrStateData%dummy)) THEN 
    ...
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstConstrStateData%dummy.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  Db_BufSz  = 0
  Int_BufSz  = 0
  Int_BufSz   = Int_BufSz   + 1     ! dummy allocated yes/no
  IF ( ALLOCATED(InData%dummy) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! dummy upper/lower bounds for each dimension
  ...
  IF ( .NOT. ALLOCATED(InData%dummy) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%dummy,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%dummy,1)
    Int_Xferred = Int_Xferred + 2

      IF (SIZE(InData%dummy)>0) ReKiBuf ( Re_Xferred:Re_Xferred+(SIZE(InData%dummy))-1 ) = PACK(InData%dummy,.TRUE.)
 ...
 ! buffers to store meshes, if any
  ...
  Db_Xferred  = 1
  Int_Xferred  = 1
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! dummy not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%dummy)) DEALLOCATE(OutData%dummy)
    ALLOCATE(OutData%dummy(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%dummy.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    ALLOCATE(mask1(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
    ...
    END IF
    mask1 = .TRUE. 
      IF (SIZE(OutData%dummy)>0) OutData%dummy = UNPACK(ReKiBuf( Re_Xferred:Re_Xferred+(SIZE(OutData%dummy))-1 ), mask1, 0.0_ReKi )
